...
/**
 * Student_upload_utilities Class File
 * PHP Version 7.1.1
 * 
 * @category   Upload Utility
 * @package    Upload Utility
 * @subpackage Student upload utility
 * @class      Student_upload_utilities
 * @desc    
 ...
 * @since   11/01/2018
 */
class Student_upload_utilities extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('Student_upload_utility');
    }
    ...
    public function upload_file() {
        ...
        if ($this->rbac->has_permission('UPLOAD_UTILITIES', 'STUDENT_UPLOAD_UTILITY')) {
            ...
            $config['on_failure_redirect'] = 'student-upload';
            $config['file'] = $_FILES;
            ...
            if ($temp_table_name) {
                ...
                $data['valid_table_config'] = $this->_get_grid_config($config['uploaded_file_heading'], 'student-upload-valid', 'valid_rec_dt');
                $data['invalid_table_config'] = $this->_get_grid_config($config['uploaded_file_heading'], 'student-upload-invalid', 'invalid_rec_dt', 'invalid');
                $this->layout->data = $data;
                $this->layout->render(array('view' => 'upload_utilities/student_upload_utilities/upload_file'));
            } else {
    ...
    public function get_temp_table_data_grid() {
        ...
        if ($this->rbac->has_permission('UPLOAD_UTILITIES', 'STUDENT_UPLOAD_UTILITY')) {
            if ($this->input->is_ajax_request()) {
                ...
                $returned_list = $this->Student_upload_utility->get_temp_table_data_dt($columns, $temp_table_name, $data, $condition);
                echo $returned_list;
                exit();
            } else {
        ...
        if ($this->rbac->has_permission('UPLOAD_UTILITIES', 'STUDENT_UPLOAD_UTILITY')) {
            ...
            if ($this->input->is_ajax_request()):
                ...
                if ($row_id):
                    ...
                    $result = $this->Student_upload_utility->delete_temp_row($row_id, $temp_table_name);
                    if ($result):
                        echo 1;
                        exit();
                    else:
    ...
    public function export_grid_data() {
        ...
        if ($this->rbac->has_permission('UPLOAD_UTILITIES', 'STUDENT_UPLOAD_UTILITY')) {
            if ($this->input->is_ajax_request()):
                ...
                $file_name = 'student_upload_' . $type . date('d_m_Y_H_i_s') . '.' . $export_type;
                $config = array(
                    'db_data' => $data['aaData'],
                    'header_rows' => $header,
    ...
    public function save_import_data() {
        if ($this->Student_upload_utility->save_import_data_db()) {
